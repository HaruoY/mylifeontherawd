...
async function main() {
  ...
  // Log diagnostico: vediamo cosa sharp legge dai metadata EXIF
  // dell'immagine scaricata da GitHub.
  const metadata = await sharp(percorsoTemp).metadata();
  console.log(`Metadata immagine: width=${metadata.width}, height=${metadata.height}, orientation=${metadata.orientation}`);

  // NOTA: non usiamo .rotate() automatico (basato su EXIF). GitHub a volte
  // ricomprime le immagini caricate nelle Issue e scrive un tag EXIF di
  // orientamento che non corrisponde ai pixel reali, causando rotazioni
  // errate. Manteniamo quindi l'orientamento fisico del file così com'è.
